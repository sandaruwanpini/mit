...
// Start class
if ( ! class_exists( 'Ambersix_Typography' ) ) {
	class Ambersix_Typography {
		public function __construct() {
			// Customizer actions
			add_action( 'customize_register', array( 'Ambersix_Typography' , 'register' ), 40 );

			// Loads Google fonts
			add_action( 'wp_enqueue_scripts', array( 'Ambersix_Typography', 'load_fonts' ) );
		
			// CSS output
			if ( is_customize_preview() ) {
				add_action( 'customize_preview_init', array( 'Ambersix_Typography', 'customize_preview_init' ) );
				add_action( 'wp_head', array( 'Ambersix_Typography', 'live_preview_styles' ), 999 );
			} else {
				add_filter( 'ambersix_custom_colors_css', array( 'Ambersix_Typography', 'head_css' ), 99 );
			}
		}
		...
		public static function elements() {
			// Return settings
			$array = apply_filters( 'ambersix_typography_settings', array(
				'body' => array(
					'label' => esc_html__( 'Body', 'ambersix' ),
					'target' => 'body',
				),
				'headings' => array(
					'label' => esc_html__( 'Headings', 'ambersix' ),
				...
				'main_menu' => array(
					'label' => esc_html__( 'Main Menu', 'ambersix' ),
					'target' => '#main-nav > ul > li > a',
					'exclude' => array( 'font-color', 'line-height' ),
				),
				'main_menu_dropdown' => array(
					'label' => esc_html__( 'Main Menu: Dropdowns', 'ambersix' ),
				...
				'featured_title' => array(
					'label' => esc_html__( 'Featured Title', 'ambersix' ),
					'target' => '#featured-title .main-title',
					'exclude' => array( 'font-color' ),
					'active_callback' => 'ambersix_cac_has_featured_title_heading',
				),
				'featured_subtitle' => array(
					'label' => esc_html__( 'Featured Sub-Title', 'ambersix' ),
					'target' => '#featured-title .sub-title',
					'exclude' => array( 'font-color' ),
					'active_callback' => 'ambersix_cac_has_featured_title_heading',
				),
				'breadcrumbs' => array(
					'label' => esc_html__( 'Breadcrumbs', 'ambersix' ),
					'target' => '#featured-title #breadcrumbs',
					'exclude' => array( 'font-color', 'line-height' ),
					'active_callback' => 'ambersix_cac_has_featured_title_breadcrumbs',
				),
				'blog_post_title' => array(
					'label' => esc_html__( 'Blog Post Title', 'ambersix' ),
					'target' => '.hentry .post-title',
					'exclude' => array( 'font-color' ),
				),
				'blog_single_post_title' => array(
					'label' => esc_html__( 'Blog Single Post Title', 'ambersix' ),
					'target' => '.is-single-post .hentry .post-title',
					'exclude' => array( 'font-color' ),
				),
				'theme_button' => array(
					'label' => esc_html__( 'Buttons', 'ambersix' ),
					'target' => '.ambersix-button, button, input[type="button"], input[type="reset"], input[type="submit"], .footer-promotion .promo-btn, .products li .product-info .button, .products li .product-info .added_to_cart, .woocommerce-page .wc-proceed-to-checkout .button, .woocommerce-page #payment #place_order',
					'exclude' => array( 'font-color', 'line-height' ),
				),
				'theme_pagination' => array(
					'label' => esc_html__( 'Pagination', 'ambersix' ),
					'target' => '.ambersix-pagination, .woocommerce-pagination',
					'exclude' => array( 'font-color', 'line-height' ),
				),
				'sidebar_widget_title' => array(
					'label' => esc_html__( 'Sidebar Widget Title', 'ambersix' ),
					'target' => '#sidebar .widget .widget-title',
				),
				'footer_widget_title' => array(
					'label' => esc_html__( 'Footer Widget Title', 'ambersix' ),
					'target' => '#footer-widgets .widget .widget-title',
					'exclude' => array( 'font-color' ),
				),
				'entry_h1' => array(
					'label' => esc_html__( 'H1', 'ambersix' ),
					'target' => 'h1',
				),
				'entry_h2' => array(
					'label' => esc_html__( 'H2', 'ambersix' ),
					'target' => 'h2',
				),
				'entry_h3' => array(
					'label' => esc_html__( 'H3', 'ambersix' ),
					'target' => 'h3',
				),
				'entry_h4' => array(
					'label' => esc_html__( 'H4', 'ambersix' ),
					'target' => 'h4',
				),
				'copyright' => array(
		...
		public static function customize_preview_init() {
			wp_enqueue_script( 'ambersix-typography-customize-preview',
				get_template_directory_uri() .'/framework/customizer/typography-customize.js',
				array( 'customize-preview' ),
				'1.0.0',
				true
			);
			wp_localize_script( 'ambersix-typography-customize-preview', 'ambersix', array(
				'googleFontsUrl' => ambersix_get_google_fonts_url()
			) );
		}
		...
		public static function register ( $wp_customize ) {
			...
			// Add General Panel
			$wp_customize->add_panel( 'ambersix_typography', array(
				'priority' => 142,
				'capability' => 'edit_theme_options',
				'title' => esc_html__( 'Typography', 'ambersix' ),
			) );

			// Add General Tab
			$wp_customize->add_section( 'ambersix_typography_general' , array(
				'title' => esc_html__( 'General', 'ambersix' ),
				'priority' => 1,
				'panel' => 'ambersix_typography',
			) );
			...
			$wp_customize->add_control( new Ambersix_Customize_Multicheck_Control( $wp_customize, 'google_font_subsets', array(
				'label' => esc_html__( 'Font Subsets', 'ambersix' ),
				'section' => 'ambersix_typography_general',
			...
			foreach( $elements as $element => $array ) {
				...
				if ( $label ) {
					...
					// Define Section
					$wp_customize->add_section( 'ambersix_typography_'. $element , array(
						'title' => $label,
						'priority' => $count,
						'panel' => 'ambersix_typography',
					...
					if ( in_array( 'font-family', $attributes ) ) {
						...
						// Add Control
						$wp_customize->add_control( new Ambersix_Fonts_Dropdown_Custom_Control( $wp_customize, $element .'_typography[font-family]', array(
								'label' => esc_html__( 'Font Family', 'ambersix' ),
								'section' => 'ambersix_typography_'. $element,
					...
					if ( in_array( 'font-weight', $attributes ) ) {
						$wp_customize->add_setting( $element .'_typography[font-weight]', array(
							'type' => 'theme_mod',
							'description' => esc_html__( 'Note: Not all Fonts support every font weight style.', 'ambersix' ),
							'sanitize_callback' => false,
							'transport' => $transport,
						) );
						$wp_customize->add_control( $element .'_typography[font-weight]', array(
							'label' => esc_html__( 'Font Weight', 'ambersix' ),
							'section' => 'ambersix_typography_'. $element,
							...
							'choices' => array(
								...
								'100' => esc_html__( 'Extra Light: 100', 'ambersix' ),
								'200' => esc_html__( 'Light: 200', 'ambersix' ),
								'300' => esc_html__( 'Book: 300', 'ambersix' ),
								'400' => esc_html__( 'Normal: 400', 'ambersix' ),
								'500' => esc_html__( 'Medium: 500', 'ambersix' ),
								'600' => esc_html__( 'Semibold: 600', 'ambersix' ),
								'700' => esc_html__( 'Bold: 700', 'ambersix' ),
								'800' => esc_html__( 'Extra Bold: 800', 'ambersix' ),
								'900' => esc_html__( 'Black: 900', 'ambersix' ),
							),
							'description' => esc_html__( 'Important: Not all fonts support every font-weight.', 'ambersix' ),
						) );
					}
					...
					if ( in_array( 'font-style', $attributes ) ) {
						...
						$wp_customize->add_control( $element .'_typography[font-style]', array(
							'label' => esc_html__( 'Font Style', 'ambersix' ),
							'section' => 'ambersix_typography_'. $element,
							...
							'choices' => array(
								'' => esc_html__( 'Default', 'ambersix' ),
								'normal' => esc_html__( 'Normal', 'ambersix' ),
								'italic' => esc_html__( 'Italic', 'ambersix' ),
							),
						) );
					}
					...
					if ( in_array( 'text-transform', $attributes ) ) {
						...
						$wp_customize->add_control( $element .'_typography[text-transform]', array(
							'label' => esc_html__( 'Text Transform', 'ambersix' ),
							'section' => 'ambersix_typography_'. $element,
							...
							'choices' => array(
								'' => esc_html__( 'Default', 'ambersix' ),
								'capitalize' => esc_html__( 'Capitalize', 'ambersix' ),
								'lowercase' => esc_html__( 'Lowercase', 'ambersix' ),
								'uppercase' => esc_html__( 'Uppercase', 'ambersix' ),
							),
						) );
					}
					...
					if ( in_array( 'font-size', $attributes ) ) {
						...
						$wp_customize->add_control( $element .'_typography[font-size]', array(
							'label' => esc_html__( 'Font Size', 'ambersix' ),
							'section' => 'ambersix_typography_'. $element,
							'settings' => $element .'_typography[font-size]',
							'priority' => 5,
							'type' => 'text',
							'description' => esc_html__( 'Value in px or em.', 'ambersix' ),
							'active_callback' => $active_callback,
						) );
					}
					...
					if ( in_array( 'font-color', $attributes ) ) {
						...
						$wp_customize->add_control( new WP_Customize_Color_Control( $wp_customize, $element .'_typography_color', array(
							'label' => esc_html__( 'Font Color', 'ambersix' ),
							'section' => 'ambersix_typography_'. $element,
					...
					if ( in_array( 'line-height', $attributes ) ) {
						...
						$wp_customize->add_control( $element .'_typography[line-height]',
							array(
								'label' => esc_html__( 'Line Height', 'ambersix' ),
								'section' => 'ambersix_typography_'. $element,
					...
					if ( in_array( 'letter-spacing', $attributes ) ) {
						...
						$wp_customize->add_control( new WP_Customize_Control( $wp_customize, $element .'_typography_letter_spacing', array(
							'label' => esc_html__( 'Letter Spacing', 'ambersix' ),
							'section' => 'ambersix_typography_'. $element,
		...
		public static function loop( $return = 'css' ) {
			...
			foreach( $elements as $element => $array ) {
				...
				$get_mod = ambersix_get_mod( $element .'_typography' );
				...
				foreach ( $attributes as $attribute ) {
					...
					if ( $val ) {
						...
						// Sanitize data
						$val = ( 'font-size' == $attribute ) ? ambersix_sanitize_data( $val, 'font_size' ) : $val;
						$val = ( 'letter-spacing' == $attribute ) ? ambersix_sanitize_data( $val, 'px' ) : $val;
		...
		public static function load_fonts() {
			...
			if ( ! empty( $fonts ) && is_array( $fonts ) ) {
				foreach ( $fonts as $font ) {
					ambersix_enqueue_google_font( $font );
				}
			}
