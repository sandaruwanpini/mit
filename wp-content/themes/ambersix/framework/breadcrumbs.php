...
function ambersix_breadcrumbs() {

    global $post;

    $home_text     = esc_html__('Home','ambersix');
    $category_text = esc_html__('Archive by Category "%s"','ambersix');
    $tax_text      = esc_html__('Archive by "%s"','ambersix');
    $tag_text      = esc_html__('Posts Tagged "%s"','ambersix');
    $author_text   = esc_html__('Articles Posted by %s','ambersix');
    $error_text    = esc_html__('Error 404','ambersix');
    $search_text   = esc_html__('Search Results for "%s" Query','ambersix');

    $blog_text   = esc_html__('Blog','ambersix');
	$project_text = esc_html__('Projects','ambersix');
	$product_text = esc_html__('Shop','ambersix');
                    ...
                    -------------------*/
                        if ( $cpt->name == 'project' ) {
                            if ( is_singular( 'project' ) ) {
                                ...
                                $this_terms = get_the_terms( $post->ID, 'project_category' );

                                $first_term         = $this_terms[0];
                                $first_term_link    = get_term_link( $first_term->term_id,'project_category' );
                                $first_term_parents = get_ancestors( $first_term->term_id,'project_category' );

                                echo '<a class="home" href="'. $home_link .'">'. $home_text .'</a>';

                                $page_id = ambersix_parse_obj_id( ambersix_get_mod( 'portfolio_page' ), 'page' );
