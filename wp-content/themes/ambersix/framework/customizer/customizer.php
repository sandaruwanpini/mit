...
// Start class
if ( ! class_exists( 'Ambersix_Customizer' ) ) {
	class Ambersix_Customizer {
		...
		public function __construct() {
			...
			$this->panels = array(
				'general' => array(
					'title' => esc_html__( 'General Options', 'ambersix' ),
				),
				'topbar' => array(
					'title' => esc_html__( 'Top Bar', 'ambersix' ),
				),
				'header' => array(
					'title' => esc_html__( 'Header', 'ambersix' ),
				),
				'featuredtitle' => array(
					'title' => esc_html__( 'Featured Title', 'ambersix' ),
				),
				'maincontent' => array(
					'title' => esc_html__( 'Main Content', 'ambersix' ),
				),
				'layout' => array(
					'title' => esc_html__( 'Layout', 'ambersix' ),
				),
				'blog' => array(
					'title' => esc_html__( 'Blog', 'ambersix' ),
				),
				'blogsingle' => array(
					'title' => esc_html__( 'Blog Single Post', 'ambersix' ),
				),
				'sidebarwidget' => array(
					'title' => esc_html__( 'Sidebar Widget', 'ambersix' ),
				),
				'projects' => array(
					'title' => esc_html__( 'Project Single', 'ambersix' ),
				),
				'shop' => array(
					'title' => esc_html__( 'Shop', 'ambersix' ),
				),
				'footer' => array(
					'title' => esc_html__( 'Footer', 'ambersix' ),
				),
				'footerwidget' => array(
					'title' => esc_html__( 'Footer Widget', 'ambersix' ),
				),
				'bottombar' => array(
					'title' => esc_html__( 'Bottom Bar', 'ambersix' ),
				),
			);
			...
			if ( ! is_admin() || is_customize_preview() ) {
				...
				if ( is_customize_preview() && $this->enable_postMessage ) {
					...
					add_action( 'ambersix_custom_colors_css', array( $this, 'head_css' ), 999 );
				}
			}
		}
		...
		public function add_sections() {
			...
			// Apply filters
			$this->sections = apply_filters( 'ambersix_customizer_sections', $this->sections );
			...
			$all_panels = $this->panels;
				...
				foreach( $all_panels as $id => $val ) {
					...
					if ( $is_section ) {
						...
						$wp_customize->add_section( 'ambersix_'. $id, array(
							'priority' => $priority,
							'title'    => $title,
						) );
					...
					// Add Panel
						...
						$wp_customize->add_panel( 'ambersix_'. $id, array(
							'priority' => $priority,
							'title'    => $title,
						) );
		...
		public function create_sections( $wp_customize ) {
			...
			foreach ( $this->sections as $section_id => $section ) {
				...
				foreach ( $settings as $setting ) {
					...
					if ( 'ambersix-heading' == $control_type ) {
					...
					} elseif ( 'ambersix-heading' == $control_type ) {
						$control_object = 'Ambersix_Customizer_Heading_Control';
					} elseif ( 'ambersix-sortable' == $control_type ) {
						$control_object = 'Ambersix_Customize_Control_Sorter';
					} elseif ( 'ambersix_textarea' == $control_type ) {
						$control_object = 'Ambersix_Customizer_Textarea_Control';
					} else {
		...
		public static function customize_controls_print_styles() { ?>
			 <style type="text/css">
				/* Icons */
				#accordion-panel-ambersix_general > h3:before,
				#accordion-panel-ambersix_typography > h3:before,
				#accordion-panel-ambersix_topbar > h3:before,
				#accordion-panel-ambersix_header > h3:before,
				#accordion-panel-ambersix_featuredtitle > h3:before,
				#accordion-panel-ambersix_maincontent > h3:before,
				#accordion-panel-ambersix_layout > h3:before,
				#accordion-panel-ambersix_blog > h3:before,
				#accordion-panel-ambersix_blogsingle > h3:before,
				#accordion-panel-ambersix_sidebarwidget > h3:before,
				#accordion-panel-ambersix_projects > h3:before,
				#accordion-panel-ambersix_shop > h3:before,
				#accordion-panel-ambersix_footer > h3:before,
				#accordion-panel-ambersix_footerwidget > h3:before,
				#accordion-panel-ambersix_bottombar > h3:before { display: inline-block; width: 20px; height: 20px; font-size: 20px; font-style: normal; vertical-align: top; text-align: center;-webkit-font-smoothing: antialiased; -moz-osx-font-smoothing: grayscale; color: #0085ba; margin-right: 6px; font-family: "dashicons"; content: "\f108"; }

				#accordion-panel-ambersix_typography > h3:before { content: "\f215" }
				#accordion-panel-ambersix_topbar > h3:before { content: "\f480" }
				#accordion-panel-ambersix_header > h3:before { content: "\f175" }
				#accordion-panel-ambersix_featuredtitle > h3:before { content: "\f209" }
				#accordion-panel-ambersix_maincontent > h3:before { content: "\f134" }
				#accordion-panel-ambersix_layout > h3:before { content: "\f535" }
				#accordion-panel-ambersix_blog > h3:before { content: "\f109" }
				#accordion-panel-ambersix_blogsingle > h3:before { content: "\f109" }
				#accordion-panel-ambersix_sidebarwidget > h3:before { content: "\f135" }
				#accordion-panel-ambersix_projects > h3:before { content: "\f481" }
				#accordion-panel-ambersix_shop > h3:before { content: "\f174" }
				#accordion-panel-ambersix_footerwidget > h3:before { content: "\f209" }
				#accordion-panel-ambersix_footer > h3:before { content: "\f535" }
				#accordion-panel-ambersix_footer > h3:before { content: "\f535" }
				#accordion-panel-ambersix_bottombar > h3:before { content: "\f138" }
				
				/* General Tweaks */
				.customize-control-select select { min-width: 100% }

				/* Slider UI */
				li.customize-control.customize-control-ambersix_slider_ui input[type=text] { width: 20%; float: left; text-align: center; }
				li.customize-control.customize-control-ambersix_slider_ui .ui-slider-horizontal.ambersix-slider-ui { float: right; width: 75%; height: 5px; margin-top: 10px; color: #414042; position: relative; border-radius: 5px; border: 1px solid #747474; border-bottom-color: #aeaeae; background-color: #cdcdcd; background: -webkit-linear-gradient(#aaaaaa, #cdcdcd); background: -o-linear-gradient(#aaaaaa, #cdcdcd); background: -moz-linear-gradient(#aaaaaa, #cdcdcd); background: linear-gradient(#aaaaaa, #cdcdcd); }
				li.customize-control.customize-control-ambersix_slider_ui .ui-slider-horizontal .ui-slider-handle { position: absolute; z-index: 2; width: 17px; height: 17px; cursor: default; top: -7px; margin-left: -10px; border-radius: 50%; border: 1px solid #9e9e9e; -webkit-box-shadow: inset 0 1px 2px rgba(0,0,0,.07); box-shadow: inset 0 1px 2px rgba(0,0,0,.07); cursor: pointer; background-color: #f5f5f5; background: -webkit-linear-gradient(#f8f8f8, #ededed); background: -o-linear-gradient(#f8f8f8, #ededed); background: -moz-linear-gradient(#f8f8f8, #ededed); background: linear-gradient(#f8f8f8, #ededed); box-shadow: 0 2px 2px rgba(0,0,0,0.24); }

				/* Sortable */
				.ambersix-sortable ul { margin-top: 10px }
				.ambersix-sortable li.ambersix-sortable-li { cursor: move; background: #fff; padding: 0; padding-left: 15px; height: 40px; line-height: 40px; white-space: nowrap; border: 1px solid #dfdfdf; text-overflow: ellipsis; -webkit-user-select: none; -moz-user-select: none; -ms-user-select: none; user-select: none; color: #222; margin-bottom: 5px; margin-top: 0; overflow: hidden; position: relative; }
				.ambersix-sortable li.ambersix-sortable-li:hover { border-color: #999; z-index: 10; }

				.ambersix-sortable li.ambersix-sortable-li .fa { display: block; position: absolute; top: 0; right: 0; width: 42px; height: 40px; line-height: 40px; margin: 0; text-align: center; font-size: 18px; }

				.ambersix-sortable li.ambersix-sortable-li .fa:after {
				...
				.ambersix-sortable li.ambersix-sortable-li .fa:before {
					content: "";
					position: absolute;
					right: 6px;
					top: 6px;
				...
				.ambersix-sortable li.ambersix-sortable-li .fa:hover:before { border-color: #0085ba; }

				.ambersix-sortable li.ambersix-sortable-li .ambersix-hide-sortee { cursor: pointer }
				.ambersix-sortable ul li:last-child { margin-bottom: 0 }
				.ambersix-sortable li.ambersix-hide .fa:after { color: #dfdfdf; }
				.ambersix-sortable li.ambersix-hide { opacity: 0.65; }

				/* Custom Heading */
				.ambersix-customizer-heading { color: #23282d; display: block; padding-top: 28px; padding-bottom: 5px; border-bottom: 2px solid #ddd; font-size: 16px; font-weight: 600; }
			 </style>
		...
		public function customize_preview_init() {
			if ( $this->enable_postMessage ) {
				wp_enqueue_script( 'ambersix-customize-preview',
		...
		public function loop_through_inline_css( $return = 'css' ) {
			...
			foreach ( $settings as $settings_array ) {
				...
				foreach ( $settings_array as $setting ) {
					...
					$theme_mod  = ambersix_get_mod( $setting_id, $default );
					...
					if ( apply_filters( 'ambersix_remove_default_mods', false ) ) {
						$get_all_mods = ambersix_get_mods();
					...
					// Sanitize data
					if ( $sanitize ) {
						$theme_mod = ambersix_sanitize_data( $theme_mod, $sanitize );
					} else {
						$theme_mod = $theme_mod;
					}
