...
class Ambersix_Customizer_Hr_Control extends WP_Customize_Control {
	public $type = 'hr';
...
class Ambersix_Customizer_Heading_Control extends WP_Customize_Control {
	public $type = 'ambersix-heading';
	public function render_content() {
		echo '<span class="ambersix-customizer-heading">' . esc_html( $this->label ) . '</span>';
	}
}

// Textarea control for the customizer
class Ambersix_Customizer_Textarea_Control extends WP_Customize_Control {
	public $type = 'ambersix_textarea';
	public $rows = '3';
	public function render_content() { ?>
		<label>
...
class Ambersix_Customize_Multicheck_Control extends WP_Customize_Control {
	public $description = '';
	public $subtitle = '';
...
// Sorter Control
class Ambersix_Customize_Control_Sorter extends WP_Customize_Control {
	...
	public function render_content() { ?>
		<div class="ambersix-sortable">
			...
			echo '<ul id="'. $this->id .'_sortable">'; ?>
				...
				foreach ( $values as $val ) :
					...
					if ( $label ) : ?>
						<li data-value="<?php echo esc_attr( $val ); ?>" class="ambersix-sortable-li">
							<?php echo esc_html( $label ); ?>
							<span class="ambersix-hide-sortee fa fa-toggle-on"></span>
						</li>
					<?php
					// End if label check
					endif;
				...
				foreach ( $choices as $val => $label ) { ?>
					<li data-value="<?php echo esc_attr( $val ); ?>" class="ambersix-sortable-li ambersix-hide">
						<?php echo esc_html( $label ); ?>
						<span class="ambersix-hide-sortee fa fa-toggle-on fa-rotate-180"></span>
					</li>
				<?php } ?>
			</ul>
		</div><!-- .ambersix-sortable -->
		...
		jQuery(document).ready( function($) {
			...
			sortableUl.on( "sortstop", function( event, ui ) {
				ambersixUpdateSortableVal();
			} );

			// Toggle classes
			sortableUl.find( 'li' ).each( function() {
				$( this ).find( '.ambersix-hide-sortee' ).on('click', function() {
					$( this ).toggleClass( 'fa-rotate-180' ).parents( 'li:eq(0)' ).toggleClass( 'ambersix-hide' );
				} );
			})
			// Update Sortable when hidding/showing items
			$( '#<?php echo esc_html( $this->id ); ?>_sortable span.ambersix-hide-sortee' ).on('click', function() {
				ambersixUpdateSortableVal();
			} );
			// Used to update the sortable input value
			function ambersixUpdateSortableVal() {
				var values = [];
				sortableUl.find( 'li' ).each( function() {
					if ( ! $( this ).hasClass( 'ambersix-hide' ) ) {
...
class Ambersix_Fonts_Dropdown_Custom_Control extends WP_Customize_Control {
	public function render_content() {
	$this_val = $this->value(); ?>
	<label>
		<span class="customize-control-title"><?php echo esc_html( $this->label ); ?></span>
			<select <?php $this->link(); ?> style="width:100%;">
			<option value="" <?php if ( ! $this_val ) echo 'selected="selected"'; ?>><?php esc_html_e( 'Default', 'ambersix' ); ?></option>
			<?php
			// Add custom fonts from child themes
			if ( function_exists( 'ambersix_add_custom_fonts' ) ) {
				$fonts = ambersix_add_custom_fonts();
				if ( $fonts && is_array( $fonts ) ) { ?>
					<optgroup label="<?php esc_html_e( 'Custom Fonts', 'ambersix' ); ?>">
			...
			$std_fonts = '';
			if ( $std_fonts = ambersix_standard_fonts() ) { ?>
				<optgroup label="<?php esc_html_e( 'Standard Fonts', 'ambersix' ); ?>">
					<?php
			...
			// Google font options
			$google_fonts = '';
			if ( $google_fonts = ambersix_google_fonts_array() ) { ?>
				<optgroup label="<?php esc_html_e( 'Google Fonts', 'ambersix' ); ?>">
					<?php
					// Loop through font options and add to select
